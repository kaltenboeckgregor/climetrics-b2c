// src/app/report/[id]/report-client.tsx
"use client";
...
export default function ReportClient({ id }: { id: string }) {
  ...
  useEffect(() => {
    try {
      ...
      const local = loadLocal<Report>(`climetrics:report:${id}`);
        ...
        <div className="mx-auto max-w-3xl p-6">
          <div className="card">
            ...
            <p className="text-sm text-slate-600">
              Die ID ist ungültig. Erstelle einen neuen Report.
            </p>
            <Link className="btn btn-primary mt-3 w-fit" href="/dashboard">
              Zum Dashboard
            </Link>
          </div>
        </div>
  ...
  // <-- ab hier existiert report garantiert:
  ...
  const share = async () => {
    ...
    // inline kompatibler Typ für die Web Share API (optional)
    const nav = navigator as Navigator & { share?: (data?: ShareData) => Promise<void> };

    if (typeof nav.share === "function") {
      await nav.share({ title: "Climetrics Report", text, url });
    } else {
          ...
          <div className="flex flex-col md:flex-row gap-6 justify-between">
            <div>
              ...
              <h1 className="text-2xl font-bold">
                {input.address || "Unbenannter Standort"}
              </h1>
              <div className="text-sm text-slate-600">
                Zeithorizont: {input.horizon} • Gebäudetyp: {input.homeType}
              </div>
            </div>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          <div className="card">
            ...
            <div className="mt-2 text-sm text-slate-600">
              Höher = besser. Steigere den Score mit Maßnahmen.
            </div>
          </div>
